...
//		System.out.println(negative);
	...
	private int maxBinarySerch(int target, int left, int right){// left-OK right-notOK
		//bottom
		if(right-left<=1){
			return left;
		}
	...
	private int memoRec(int i, int j){
		...
		// recursion
			...
			if (j<cost){
				...
				ans = Math.max(memoRec(i+1,j),
						memoRec(i+1,j-cost)+value);
			}
		}
		// record 
		recordMemo(i,j,ans);
		...
		for(int k=0;update && k<edge.length;k++){
			for(int i=0;i<edge.length;i++){ // for all edges
				for(int j=0;j<edge[i].length;j++){
					if(d[i]+edge[i][j]<d[j]){	d[j] = d[i] + edge[i][j];
						update = true;
...
//		return false;
	...
	private int[] solveDijkstra(int[][] edge,int start){// none - INF
		...
		PriorityQueue<Integer[]> que = 
			new PriorityQueue<>((x1,x2)->Integer.compare(x1[1],x2[1]));
